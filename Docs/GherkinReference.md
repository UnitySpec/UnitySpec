# Gherkin Reference
> Also take a look at the documentation by [SpecFlow](https://specflow.org/learn/gherkin/) and [Cucumber](https://cucumber.io/docs/gherkin/reference/)
...
These description lines are ignored by UnityFlow at runtime, but are available for reporting (They are included by default in html reports).
...
The name and the optional description have no special meaning to UnityFlow. Their purpose is to provide
...
UnityFlow generates categories from the tags. 
The generated category name is the same as the tag's name, but without the leading @. 
You can filter and group the tests to be executed using these unit test categories. 
For example, you can tag crucial tests with @important, and then execute these tests more frequently.

You can still use tags to implement special logic for tagged scenarios in bindings by querying the `ScenarioContext.Current.ScenarioInfo.Tags` property.

Scenario and Feature level tags are available by querying the `ScenarioInfo.ScenarioAndFeatureTags` property.

UnityFlow treats the `@ignore` tag as a special tag. UnityFlow generates an [ignored unit test](../Execution/Test-Results.html#ignored-tests) method from scenarios with this tag.
...
The `Rule` keyword has no effect on the files generated by UnityFlow.
...
UnityFlow executes each step in a scenario one at a time, in the sequence you’ve written them in.
When UnityFlow tries to execute a step, it looks for a matching step definition to execute.
...
UnityFlow considers the following steps duplicates:
...
When UnityFlow executes a `Given` step, it will configure the system to be in a well-defined state,
...
UnityFlow will replace these parameters with values from the table *before* it tries
...
**Hint:** In certain cases, when generating method names using the regular expression method, UnityFlow is unable to generate the correct parameter signatures for unit test logic methods without a little help. Placing single quotation marks (`'`) around placeholders (eg. `'<placeholder>'`)improves UnityFlow's ability to parse the scenario outline and generate more accurate regular expressions and test method signatures.
...
Using a language other than English has not been tested in UnityFlow.
