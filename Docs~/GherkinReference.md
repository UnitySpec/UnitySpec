...
These description lines are ignored by UnitySpec at runtime, but are available for reporting (They are included by default in html reports).
...
The name and the optional description have no special meaning to UnitySpec. Their purpose is to provide
...
UnitySpec generates categories from the tags. 
...
UnitySpec treats the `@ignore` tag as a special tag. UnitySpec generates an [ignored unit test](../Execution/Test-Results.html#ignored-tests) method from scenarios with this tag.
...
The `Rule` keyword has no effect on the files generated by UnitySpec.
...
UnitySpec considers the following steps duplicates:
...
When UnitySpec executes a `Given` step, it will configure the system to be in a well-defined state,
...
UnitySpec will replace these parameters with values from the table *before* it tries
...
**Hint:** In certain cases, when generating method names using the regular expression method, UnitySpec is unable to generate the correct parameter signatures for unit test logic methods without a little help. Placing single quotation marks (`'`) around placeholders (eg. `'<placeholder>'`)improves UnitySpec's ability to parse the scenario outline and generate more accurate regular expressions and test method signatures.
